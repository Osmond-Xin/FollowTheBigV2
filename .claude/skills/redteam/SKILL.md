...
- 同一分支红队 **≤ 5 轮**，脚本按 `.redteam/rounds/<分支>` 计数，满了退出码 4：停，去评估或找人裁定。多轮红队会飘、会带着主思路走。
- 每个 PR 默认只跑 **1 轮**；攻击面只查五条硬约束（深模块 · 结构化互相调用有约束 · 同一功能只有一个入口 · 单一职责 · 不重复造轮子 / 新功能必须调用已有功能）+ 正确性。其余发现记周检清单，不当场修。
