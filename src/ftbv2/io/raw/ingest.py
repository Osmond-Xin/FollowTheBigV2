...
- 原子写：先写临时文件再 os.replace；写前清扫同一目标残留的 .tmp；幂等判据 = manifest 三 stream 齐全，不是「某个文件存在」；
    ...
    *,
    ...
    try:
        ...
        receipt = IngestReceipt(
            day, archive, archive_sha, tuple(prefixes), _sevenzip_version(),
            tuple(StreamReceipt(r.stream, r.n_symbols, r.n_rows_csv, r.n_rows_parquet, r.header, sizes[r.stream], r.sha256_csv)
                  for r in receipts),
            dropped,
        )
...
def _frame_segment(digest: hashlib._Hash, payload: bytes) -> None:
...
def _run_7zz(*args: str) -> str:
    try:
        return subprocess.run(["7zz", *args], capture_output=True, text=True, check=True).stdout
...
def _extract(archive: Path, scratch: Path) -> None:
    _run_7zz("x", "-y", "-bso0", "-bsp0", f"-o{scratch}", str(archive))
...
def _discover_csvs(scratch: Path, day: Day, prefixes: tuple[str, ...]) -> tuple[dict[Stream, list[tuple[str, Path]]], dict[str, int]]:
    ...
    for path in sorted(scratch.rglob("*.csv")):
        ...
        if stream is None:
            raise RuntimeError(f"归档里有未登记的 CSV 文件名：{name}")
...
def _parse_body(body: bytes, symbol: str, ncols: int, n_rows: int) -> pl.DataFrame:
    ...
    if n_rows == 0:
        ...
        frame = pl.read_csv(
            io.BytesIO(body), has_header=False, encoding="utf8-lossy",
...
def _write_streams(root: Path, day: Day, frames: dict[Stream, pl.DataFrame]) -> dict[Stream, int]:
    sizes: dict[Stream, int] = {}
    for stream, frame in frames.items():
        ...
        sizes[stream] = target.stat().st_size
    return sizes


def _atomic_write(target: Path, write) -> None:
    """先清扫同一目标残留的 .tmp（上次崩溃遗留），再写临时文件并 os.replace。"""
