...
REFILL_AFTER_FILL = EventSpec(
    ...
    relation=Relation(
        ...
        invariants=(Inv.REFILL_STRICTLY_AFTER_FILL, Inv.SLICE_EXCEEDS_MIN_LOT),
        doc=(
            "三者按时序咬合并至少重复一轮：委托 → 被成交吃完 → 同 (price, vol) 的新委托出现，"
            "**且每片超过一手**。"
            "「补单必须严格发生在成交之后」是冰山的机制——上一版只要求「同价同量相邻两笔」，"
            "那在密集报单里满地都是，与是否成交无关，切出来的不是冰山。"
            "「每片超过一手」是 2026-09-03 实测加上去的：不加它，实测 218.92 条/(标的·日)，"
            "而每片量中位**在每一个可见档位桶、每一个轮数桶里都是 100 股**——"
            "**一手不能再切，所以一手的重复挂单不是切片的证据**"
        ),
    ),
    open_rule=(
        "某 (side, price) 上一笔**超过一手**的委托被成交耗尽后，同 (price, vol) 的新委托出现——"
        "即第一次完成「成交 → 补单」。一轮即成立：「重复」的词义下限是 2 笔委托，不是阈值。"
        "一手取 `core.raw.LOT_SIZE`：它是**交易所定的最小委托单位**，与十档发布范围同类，"
        "不是我们从分布里挑出来的一个数"
    ),
    close_rule=(
        "出现价或量与本组不同的本方委托，或该价位被穿过，或**撞上交易所的状态边界**"
        "（午休 · 收盘集合竞价开始 · 停牌 · 该标的当日结束）。**不得写「隔了 T 久没补就结组」**："
        "状态边界是交易所定的结构，不是我们掐的表（红队 2026-09-03 工程严重 2）。"
        "代价是有人整天在同一价位补同样的量时这一组横跨半天——那本来就是事实，"
    ...
    measures=(
        ...
        Measure("slice_vol", "int", "股",
                "每片的委托量。本组内恒定——它是成组的条件之一。不变量已要求它超过一手", _VAR),
        Measure("n_refills", "int", "次", "完成了几轮「成交 → 补单」。几轮算多是判断，这里只记几轮", _VAR),
        Measure("total_filled", "int", "股", "本组累计被成交的量。它是 n_refills × slice_vol 的实测校验", _VAR),
        Measure("span_ms", "int", "毫秒", "首片到末片的毫秒跨度。组不是区间：中间可能夹着别人的委托", _VAR),
        Measure(
            "close_reason", "enum", "枚举",
            "本组因为什么结束：换价换量 / 价位被穿 / 撞上状态边界。"
...
**关联率上一版记错了**：「上交所单边 57% / 双边 14%」量的是混了主被动的聚合。
冰山要的是**被动方**那一边（「本方委托」就是挂在那里被吃的那一笔），
实测 SH 99.89% / SZ 99.91%——两个市场都可靠。分交易所报仍然做，理由是让这件事可核。"""
