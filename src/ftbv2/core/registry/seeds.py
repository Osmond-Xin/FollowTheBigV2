...
这不是「我们自己复核了一遍」——独立上下文、独立判据、结论与作者相左，才算对照。"""
...
REFILL_AFTER_FILL = EventSpec(
    ...
    relation=Relation(
        ...
        invariants=(Inv.REFILL_STRICTLY_AFTER_FILL,),
        doc=(
            "三者按时序咬合并至少重复一轮：委托 → 被成交吃完 → 同 (price, vol) 的新委托出现。"
            "**补单必须严格发生在成交之后**——这才是冰山的机制。上一版只要求「同价同量相邻两笔」，"
    ...
    measures=(
        _SIDE,
        _PRICE,
        Measure("slice_vol", "int", "股", "每片的委托量。本组内恒定——它是成组的条件之一", _VAR),
...
⚠️ 未实测。红队指出上交所 trades→orders 关联率只有单边 57% / 双边 14%（深交所 86% / 72%），
「某一笔本方委托被成交耗尽」在上交所不可靠——实测时必须**分交易所报**，不得合成一个数。"""
